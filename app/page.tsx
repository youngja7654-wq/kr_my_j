...
const calendarItems: Record<number,{text:string;kind:Kind}[]> = {
  3:[{text:"다이맥스 마그마",kind:"max"}], 4:[{text:"GO패스 시작",kind:"event"},{text:"서머 마라톤",kind:"event"}], 5:[{text:"유크시 변경",kind:"raid"},{text:"레이드 아워",kind:"raid"}], 6:[{text:"딜리버드 스포트라이트",kind:"event"}], 8:[{text:"불꽃과 얼음 부화 데이",kind:"event"}], 10:[{text:"다이맥스 메탕",kind:"max"}], 12:[{text:"그란돈 변경",kind:"raid"},{text:"레이드 아워",kind:"raid"}], 13:[{text:"개무소 스포트라이트",kind:"event"}], 16:[{text:"훔처우 커뮤니티 데이",kind:"event"}], 17:[{text:"다이맥스 잉어킹",kind:"max"}], 18:[{text:"워터 페스티벌",kind:"event"}], 19:[{text:"루나아라 변경",kind:"raid"},{text:"레이드 아워",kind:"raid"}], 20:[{text:"잉어킹 스포트라이트",kind:"event"}], 22:[{text:"메가 아쿠스타 레이드 데이",kind:"raid"}], 24:[{text:"다이맥스 카포에라",kind:"max"}], 26:[{text:"레지 3종 변경",kind:"raid"},{text:"레이드 아워",kind:"raid"}], 27:[{text:"망키 스포트라이트",kind:"event"}], 31:[{text:"다이맥스 이브이",kind:"max"}],
};
...
export default function Home(){
  ...
  return <main>
    ...
    <section className="content" id="calendar"><div className="section-heading"><div><p className="eyebrow dark"><span/> Monthly schedule</p><h2>2026년 8월</h2></div><div className="view-switch" aria-label="보기 방식"><button className={view==="calendar"?"active":""} onClick={()=>setView("calendar")}>달력</button><button className={view==="list"?"active":""} onClick={()=>setView("list")}>목록</button></div></div>
      ...
      {view==="calendar"?<div className="calendar-shell"><div className="weekdays">{["일","월","화","수","목","금","토"].map(d=><span key={d}>{d}</span>)}</div><div className="calendar-grid">{days.map((day,index)=>{const inMonth=day>=1&&day<=31;const shown=inMonth?(calendarItems[day]||[]).filter(i=>filter==="all"||i.kind===filter):[];const display=day<=0?31+day:day>31?day-31:day;return <div className={`day-cell ${inMonth?"":"muted"} ${day===13?"today":""}`} key={index}><span className="day-number">{display}</span>{shown.map((i,n)=><span className={`calendar-chip ${i.kind}`} key={`${i.text}-${n}`}>{i.text}</span>)}</div>})}</div></div>:<ScheduleList items={visible}/>}<div className="legend"><span><i className="event"/> 이벤트</span><span><i className="raid"/> 레이드</span><span><i className="max"/> 맥스배틀</span><span className="notice">일정은 현지 시간 기준이며 변경될 수 있습니다.</span></div>
    </section>
